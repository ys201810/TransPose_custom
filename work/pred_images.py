...
def main(image_bgr, save_path, box_model, model):
    ...
    # 最も大きいボックスのみ残す
    largest_index = np.argmax([pred_box[1][0] - pred_box[0][0] for pred_box in pred_boxes])
    pred_boxes = [pred_boxes[largest_index]]
    # pred_boxesは[(x1, y1), (x2, y2)]なので、そこからx, y, w, hを作成
    box_x = pred_boxes[0][0][0]
    box_y = pred_boxes[0][0][1]
    box_w = pred_boxes[0][1][0] - pred_boxes[0][0][0]
    box_h = pred_boxes[0][1][1] - pred_boxes[0][0][1]
    ...
    return pose_preds, [box_x, box_y, box_w, box_h]
...
if __name__ == '__main__':
    ...
    for i, target in enumerate(targets):
        ...
        pred_results = {}
        pred_results['keypoints'] = pose_preds
        pred_results['bbox'] = box_pred
        results[target] = pred_results

    with open('data/keypoint_results.pkl', 'wb') as outf:
